...
from ironlib.orbit.tasks.common.sim_utils import MultiAssetCfg, spawn_fixed_number_of_multi_object_sdf
...
target_init_pos = [(0.0, 0.0, 1.25)] * len(obj_usd_files)  # Stand placement
...
class MultiAssetsScene:
    ...
    def rigid_object(cls):
        return RigidObjectWithAttachmentCfg(
            ...
            attachments=[
                JointAttachmentCfg(
                    joint_type="fixed",
                    prim_path="{ENV_REGEX_NS}/" + f"cube_{i}" + SuctionGrippers.default_attachment_path(),
                    attached_link_idx=None,
                    local_pos0=SuctionGrippers.local_pos0[i],
                    local_pos1=SuctionGrippers.local_pos1[i],
                    name=f"attachment_{i}",
                )
                for i in range(SuctionGrippers.N_GRIPPERS)
            ],
            spawn=MultiAssetCfg(
    ...
    def cube_params(cls, index: int, rigid: bool = True):
        return {
            ...
            "init_state": MultiAssetInitialStatesCfg(
                pos=[SuctionGrippers.grippers_init_state[index] for _ in range(MultiGeometryScene.num_geometries)],
            ),
        }
...
@configclass
class RigidInsertionMultiEnvCfg(RigidInsertionEnvCfg):
