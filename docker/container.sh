...
case $mode in
    start)
        ...
        cp $SCRIPT_DIR/../.dockerignore $SCRIPT_DIR/../../  #  copy .dockerignore to the workspace directory
        pushd ${SCRIPT_DIR} > /dev/null 2>&1
        docker compose --file docker-compose.yaml up --detach --build --remove-orphans
        popd > /dev/null 2>&1
        ;;
    resume)
        echo "[INFO] Reuse the builded docker image and starting the container in the background..."
        pushd ${SCRIPT_DIR} > /dev/null 2>&1
    ...
    push)
        ...
        if [ -f $SCRIPT_DIR/.env ]; then
            ...
            tar -zcvf /$SCRIPT_DIR/exports/orbit.tar orbit.sif
